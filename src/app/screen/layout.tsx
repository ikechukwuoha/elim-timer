...
export default function BigScreen() {
  ...
  // ── Pusher subscription (cross-device real-time) ──────────
  useEffect(() => {
    const key     = process.env.NEXT_PUBLIC_PUSHER_KEY
    const cluster = process.env.NEXT_PUBLIC_PUSHER_CLUSTER

    if (!key || !cluster) {
      console.warn('Pusher env vars not set — cross-device sync disabled')
      return
    }
    ...
    import('pusher-js').then(({ default: Pusher }) => {
      ...
      channel.bind('TIMER_UPDATE', (data: TimerState) => {
        setTimerState(data)
        // Also save to localStorage so refreshing the page keeps last state
        localStorage.setItem('elim_timer_state', JSON.stringify(data))
      })

      channel.bind('PRESENT_UPDATE', (data: PresentState) => {
        setPresentState(data)
        localStorage.setItem('elim_present_state', JSON.stringify(data))
      })
    })
  ...
  if (!timerState || !presentState) return (
    <div style={{ width:'100vw', height:'100vh', background:'#000',
      ...
      color:'#333', fontSize:24, fontFamily:'system-ui, sans-serif' }}>
      Connecting…
    </div>
  )
  ...
  return (
    <>
      <style>{`
        ...
        .timer-crit    {animation:critical-pulse .7s  ease-in-out infinite}
      ...
      <div style={{
        ...
        {/* Grid overlay */}
        <div className={isTimerMode ? (color==='green'?'bg-green':color==='yellow'?'bg-yellow':'bg-red') : ''}
          style={{
        ...
        <header style={{
          ...
          {/* Connection + Live status — top right */}
          <div style={{ display:'flex', alignItems:'center', gap:12 }}>
            {/* Pusher connection dot */}
            <div style={{
        ...
        <main style={{
          ...
          {isTimerMode && (
            <>
              <p className="label-anim" style={{
                ...
                margin:'0 0 8px', textShadow:`0 0 20px ${theme.glowColor}`,
              }}>{statusTxt}</p>

              <p
                className={isCrit?'timer-crit':color==='yellow'?'timer-warning':'timer-calm'}
                style={{
                  fontFamily:'var(--font-bebas), cursive',
                  fontSize:'min(38vw,38vh)', lineHeight:0.85,
              ...
              >{formatTime(timerState.remaining)}</p>

              <div style={{
                width:180, height:2, margin:'min(2.5vw,2.5vh) 0',
              ...
              <p style={{
                fontFamily:'var(--font-cinzel), serif',
                fontSize:'min(6vw,6vh)', fontWeight:600, color:'#fff',
              ...
              <p style={{
                marginTop:'min(1.2vw,1.2vh)', fontSize:'min(1.8vw,1.8vh)',
                fontWeight:400, color:'#fff', letterSpacing:'0.22em',
                textTransform:'uppercase', textAlign:'center',
                textShadow:`0 0 12px ${theme.glowColor}`, opacity:0.9,
              }}>{current.duration} minutes allocated</p>
            </>
          )}
          ...
          {/* BLANK */}
          {isBlank && (
            <p style={{ color:'rgba(255,255,255,0.06)', fontSize:14, letterSpacing:'0.2em', textTransform:'uppercase' }}>
              Standby
            </p>
          )}
        </main>
