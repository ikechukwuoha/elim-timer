...
// ── Preset list (shown instantly before any network call) ─────
...
export async function fetchTranslations(): Promise<BibleTranslation[]> {
  ...
  // 1. Local DB (fastest, works offline)
  try {
    const res = await fetch(`${LOCAL_BASE}/translations`)
    if (res.ok) {
      const data = await res.json() as BibleTranslation[]
  ...
  // 2. bolls.life
  try {
    const res  = await fetch(`${BOLLS_BASE}/static/bolls/app/views/languages.json`)
...
export async function fetchBooks(translationId: string): Promise<BibleBook[]> {
  ...
  // 1. Local DB
  try {
    const res = await fetch(`${LOCAL_BASE}/books/${translationId}`)
    if (res.ok) {
  ...
  // 2. bolls.life
  try {
    const res  = await fetch(`${BOLLS_BASE}/get-books/${translationId}/`)
    const data = await res.json() as BibleBook[]
    if (Array.isArray(data) && data.length > 0) {
      _booksCache[translationId] = data
      return data
...
export async function fetchChapter(
  ...
  // 1. Local DB
  try {
    const res = await fetch(`${LOCAL_BASE}/text/${translationId}/${bookId}/${chapter}`)
    if (res.ok) {
      ...
      if (Array.isArray(data) && data.length > 0) {
        const result: BibleChapter = {
          book:   bookName,
          chapter,
          verses: data.map(v => ({ verse: v.verse, text: stripHtml(v.text) })),
        }
  ...
  // 2. bolls.life
  try {
    const res  = await fetch(`${BOLLS_BASE}/get-text/${translationId}/${bookId}/${chapter}/`)
...
export const BOOK_ID_MAP: Record<string, string> = {
  ...
  'Numbers': '4',         'Deuteronomy': '5',       'Joshua': '6',
  'Judges': '7',          'Ruth': '8',              '1 Samuel': '9',
